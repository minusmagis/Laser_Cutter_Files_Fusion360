...
properties = {
  ...
  rapidTravelZ: 100,
  laserGuide: "M106 S4",
  laserVapourize: "M106 S255",
  laserThrough: "M106 S125",
  laserOFF: "M107"
};
...
function onOpen() {
  ...
  writeln(";By Tech2C");
  writeln(";Modified by minusmagis");
...
function onSection() {
  ...
  if (currentSection.getType() == TYPE_JET) {
    if(currentSection.jetMode == 0) {
    cuttingMode = properties.laserThrough;
    writeln("");
	writeWords(properties.laserOFF, "         ;Laser/Fan OFF");
    writeln("");
  	writeln(";Here we pause to allow the user to prepare for the real engraving and we notify him");
    writeln("");
    writeln("M117 Safety Glasses ON!!");
 	writeln("M0 Safety 1st Lasers 2nd;");
    }
	else if(currentSection.jetMode == 1) {cuttingMode = properties.laserGuide}
	else if(currentSection.jetMode == 2) {
	cuttingMode = properties.laserVapourize;
	writeln("");
	writeWords(properties.laserOFF, "         ;Laser/Fan OFF");
    writeln("");
  	writeln(";Here we pause to allow the user to prepare for the real engraving and we notify him");
